...
export const updateAppointment = async ({appointmentId, userId, appointment, type}: UpdateAppointmentParams) => {
    try {
        ...
        const smsMessage = `
        Hi, it is CarePulse.
        ${type === 'schedule' ? `Your appointment has been scheduled for ${formatDateTime(appointment.schedule)}` : `We regret to inform you that your appointment has been cancelled. For the following reason: ${appointment.cancellationReason}`
        }`
